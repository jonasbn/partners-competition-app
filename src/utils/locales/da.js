// Danish translations
export default {
  "app": {
    "title": "Partners Konkurrence App",
    "footer": "Partners Konkurrence App © {{year}}",
    "createdWith": "Skabt med GitHub Copilot og Claude Sonnet 3.7"
  },
  "language": {
    "en": "Engelsk",
    "da": "Dansk"
  },
  "summary": {
    "leadingPlayer": {
      "title": "Førende Spiller",
      "score": "Nuværende Score",
      "maxScore": "Maksimum muligt: {{score}} ({{games}} spil)"
    },
    "mostWinningTeam": {
      "title": "Mest Vindende Hold",
      "stats": "Vandt {{wins}} ud af {{games}} spil ({{rate}}%)",
      "combinations": "Ud af {{count}} mulige holdkombinationer"
    },
    "gameStatistics": {
      "title": "Spil Statistikker",
      "totalGames": "Samlede Spil",
      "possibleCombinations": "Holdkombinationer Tilgængelige",
      "averageGameScore": "Gennemsnitlig Spil Score"
    }
  },
  "theme": {
    "dark": "Mørk",
    "light": "Lys",
    "toggle": "Skift Tema"
  },
  "leaderboard": {
    "title": "Resultattavle",
    "position": "Placering",
    "player": "Spiller",
    "score": "Score",
    "games": "Spil",
    "wins": "Sejre",
    "winRate": "Sejrs Rate",
    "avgScore": "Gns Score",
    "rank": "Placering",
    "points": "Point",
    "avg": "Gns"
  },
  "teamStatistics": {
    "title": "Hold Statistikker",
    "team": "Hold",
    "gamesPlayed": "Spil",
    "avgScore": "Gns Score",
    "firstPlace": "1.",
    "secondPlace": "2.", 
    "thirdPlace": "3.",
    "winRate": "Sejrs Rate"
  },
  "playerPerformance": {
    "title": "Spiller Præstations Analyse",
    "totalScore": "Total Score",
    "averageScore": "Gennemsnits Score",
    "bestScore": "Bedste Score",
    "worstScore": "Værste Score",
    "consistency": "Konsistens",
    "trend": "Trend"
  },
  "gamesList": {
    "title": "Spil Historie",
    "date": "Dato",
    "teams": "Hold",
    "winner": "Vinder",
    "score": "Score",
    "sort": "Sortér",
    "team": "Hold",
    "teamsScores": "Hold & Score",
    "place": {
      "1st": "1.",
      "2nd": "2.", 
      "3rd": "3."
    },
    "points": {
      "one": "point",
      "other": "point",
      "first": "1. Plads",
      "second": "2. Plads",
      "third": "3. Plads"
    },
    "game": "Spil {{number}}",
    "recent": "Seneste Spil",
    "showAll": "Vis Alle Spil",
    "showRecent": "Vis Seneste Spil",
    "noGames": "Ingen spil fundet",
    "loading": "Indlæser spil...",
    "error": "Fejl ved indlæsning af spil",
    "summary": "{{total}} spil spillet over {{days}} dage"
  },
  "playerStats": {
    "title": "Spillerstatistik",
    "playerTypes": {
      "rookie": "Rookie",
      "pro": "Pro", 
      "veteran": "Veteran",
      "dominator": "Dominator",
      "legend": "Legende"
    },
    "metrics": {
      "gamesPlayed": "Spil Spillet",
      "totalWins": "Samlede Sejre",
      "winRate": "Sejrs Rate",
      "averageScore": "Gennemsnits Score",
      "bestStreak": "Bedste Række",
      "currentStreak": "Nuværende Række"
    },
    "insights": {
      "strongPlayer": "Stærk spiller med konsistent præstation",
      "improvingPlayer": "Spiller viser forbedring over tid",
      "inconsistentPlayer": "Inkonsistent præstation - plads til forbedring",
      "dominantPlayer": "Dominerende spiller - konsistent bedre end andre",
      "newPlayer": "Ny spiller - opbygger erfaring"
    },
    "loading": "Indlæser spiller statistikker...",
    "error": "Fejl ved indlæsning af spiller statistikker",
    "noData": "Ingen spiller data tilgængelig"
  },
  "summaryCards": {
    "currentLeader": {
      "title": "Nuværende Leder",
      "description": "Fører konkurrencen"
    },
    "gameStatistics": {
      "title": "Spil Statistikker", 
      "totalGames": "Samlede Spil",
      "maxPossible": "Max Mulige Point",
      "calculation": "{{games}} spil × 3 point = {{points}} point"
    },
    "bestTeam": {
      "title": "Bedste Hold",
      "description": "Mest succesfulde partnerskab"
    },
    "teamCombinations": {
      "title": "Hold Kombinationer",
      "description": "{{used}} af {{total}} kombinationer brugt",
      "remaining": "{{remaining}} kombinationer tilbage"
    },
    "loading": "Indlæser statistikker...",
    "error": "Fejl ved indlæsning af statistikker"
  },
  "errorBoundary": {
    "title": "Noget gik galt",
    "message": "Vi beklager, men noget uventet skete. Prøv venligst at genindlæse siden.",
    "tryAgain": "Prøv Igen"
  },
  "gamesCalendar": {
    "title": "Spil Kalender",
    "stats": {
      "totalGames": "Samlede Spil",
      "gameDays": "Spil Dage", 
      "averagePerDay": "Gennemsnit pr. Dag",
      "mostActiveDay": "Mest Aktive Dag"
    },
    "timeline": {
      "title": "Aktivitets Tidslinje",
      "legend": "Aktivitets Niveau",
      "noActivity": "Ingen Aktivitet",
      "tooltip": "{{games}} spil den {{date}}"
    },
    "levels": {
      "none": "Ingen",
      "low": "Lav",
      "medium": "Medium", 
      "high": "Høj",
      "veryHigh": "Meget Høj"
    },
    "recentGames": {
      "title": "Seneste Spil",
      "date": "Dato",
      "games": "Spil",
      "winner": "Vinder",
      "noGames": "Keine seneste spil"
    }
  },
  "charts": {
    "playerStats": {
      "title": "Spillerstatistik",
      "axisLabels": {
        "player": "Spiller",
        "score": "Score"
      }
    },
    "playerPerformance": {
      "title": "Spiller Præstation"
    },
    "gamesCalendar": {
      "title": "Spil Kalender"
    },
    "teamCombinations": {
      "title": "Hold Kombinationer"
    }
  }
};